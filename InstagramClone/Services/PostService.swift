...
class PostService {
    ...
    static func fetchPosts() async throws -> [Post] {
        let snapshot = try await FirestoreConstants.postsCollection.getDocuments()
    ...
    static func fetchPost(uid: String) async throws -> Post {
        let snapshot = try await FirestoreConstants.postsCollection.document(uid).getDocument()
